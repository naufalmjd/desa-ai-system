    ...
    <div class="col-lg-8">
        <div class="card border-0">
            <div class="card-header bg-transparent py-3 border-0 d-flex align-items-center gap-2">
                ...
                <span class="fs-6 fw-bold text-white">Formulir Pembaruan Pengguna</span>
            </div>
            <div class="card-body p-4">
                <form action="<?= APP_URL ?>/superadmin/user/update/<?= $user['id'] ?>" method="POST" id="userForm">
                    ...
                    <!-- Password -->
                    <div class="mb-3">
                        <label for="password" class="form-label text-white-50 fw-semibold" style="font-size: 0.82rem;">Kata Sandi Baru (Kosongkan jika tidak diubah)</label>
                        <div class="input-group">
                            <span class="input-group-text bg-transparent border-secondary text-muted"><i class="bi bi-lock"></i></span>
                            <input type="password" id="password" name="password" class="form-control" placeholder="Masukkan kata sandi baru..." autocomplete="new-password">
                            <button class="btn btn-outline-secondary" type="button" id="togglePassword"><i class="bi bi-eye-slash"></i></button>
                        </div>
                        <small class="text-muted" style="font-size: 0.72rem;">Minimal 6 karakter, biarkan kosong agar password lama tetap digunakan.</small>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="role_id" name="role_id" class="form-select" required>
                                ...
                                <option value="<?= $r['id'] ?>" <?= $user['role_id'] === $r['id'] ? 'selected' : '' ?>><?= htmlspecialchars($r['label']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <div class="d-flex gap-2 justify-content-end">
                        ...
                        <button type="submit" class="btn btn-primary px-4 py-2">
                            <i class="bi bi-check-circle me-1"></i> Perbarui User
                        </button>
                    </div>
                </form>
            </div>
